...
var Blaize_Companion_Customize_Control_Tabs = function ( $ ) {
	'use strict';
		...
		function () {
			...
			$( '.blaize-tabs-control' ).each(
				function () {
					...
					var shownCtrls = $( this ).find( '.blaize-customizer-tab > input:checked' ).data( 'controls' );
					showControls( customizerSection, shownCtrls );
				}
			);

			$( '.blaize-customizer-tab > label' ).on(
				'click', function () {
					var customizerSection = $( this ).closest( '.accordion-section' );
...
Blaize_Companion_Customize_Control_Tabs( jQuery );
